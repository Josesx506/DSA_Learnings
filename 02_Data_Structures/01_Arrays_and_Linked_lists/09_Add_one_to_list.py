...
would be provided as `arr = [1, 2, 3]`. Add one to the number and return the output in the 
form of a new list. 
...
**Note** - Try proposing a non-recursive solution. We will see a recursive solution in the next 
lesson "Recursion". 
"""

# My implementation. This is wrong becuase datatype conversions cannot be implemented easily in other languages
def add_one(arr):
    """
    :param: arr - list of digits representing some number x
    return a list with digits represengint (x + 1)
    """
    value = "".join([str(s) for s in arr]) # Convert the array into a string
    # Convert the string into a value to increment it and return the type as string
    value = str(int(value)+1) 
    
    out = [int(i) for i in value] # Create an output of integers
    return out
